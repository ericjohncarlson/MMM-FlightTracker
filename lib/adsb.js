...
class Adsb extends EventEmitter {
    ...
    _fetchRoutes(aircraftList) {
        ...
        const req = https.request(options, (res) => {
            ...
            res.on('end', () => {
                try {
                    ...
                    console.debug('Route lookup parse error:', e.message);
                }
            });
        });
        ...
        req.on('error', (e) => {
            // Route lookup failed, not critical
            console.debug('Route lookup failed:', e.message);
        });
    ...
    _processRoutes(routes) {
        // The API returns an object with callsigns as keys
        if (typeof routes === 'object' && routes !== null) {
            for (const [callsign, routeData] of Object.entries(routes)) {
                if (routeData && routeData._airports && routeData._airports.length >= 2) {
                    const origin = routeData._airports[0];
                    const dest = routeData._airports[routeData._airports.length - 1];
