...
const ADSB_IM_ROUTESET_URL = 'https://adsb.im/api/0/routeset';
...
class Adsb extends EventEmitter {
    ...
    constructor() {
        ...
        this.mode = null;
        this.routeCache = new Map(); // Cache routes by callsign: { route, timestamp }
        this.routeCacheTTL = 15 * 60 * 1000; // 15 minute TTL for route cache
    }
    ...
    // Get a cached route if it exists and hasn't expired
    _getCachedRoute(callsign) {
        const cached = this.routeCache.get(callsign);
        if (!cached) {
            return null;
        }
        // Check if cache entry has expired
        if (Date.now() - cached.timestamp > this.routeCacheTTL) {
    ...
    // Check if a route needs to be fetched (not cached or expired)
    _needsRouteFetch(callsign) {
        const cached = this.routeCache.get(callsign);
        if (!cached) {
            return true;
        }
        // Check if cache entry has expired
        return Date.now() - cached.timestamp > this.routeCacheTTL;
    }
    ...
    _processTar1090Data(json, enableRoutes) {
        ...
        this.aircrafts = validAircraft.map(ac => ({
            ...
            rssi: ac.rssi,
            seen: ac.seen,
            route: this._getCachedRoute((ac.flight || '').trim())
        }));
    ...
    _fetchRoutes(aircraftList) {
        ...
        const needRoutes = aircraftList.filter(ac => {
            ...
            // Has callsign, has position, needs fetch (not cached or expired), looks like a flight number (has digits)
            return callsign &&
                   ac.lat &&
                   ac.lon &&
                   this._needsRouteFetch(callsign) &&
                   /\d/.test(callsign);
        });
        ...
        const requestedCallsigns = new Set(planes.map(p => p.callsign));
        console.log('Route API request callsigns:', Array.from(requestedCallsigns).join(', '));
        ...
        const req = https.request(options, (res) => {
            ...
            res.on('end', () => {
                ...
                try {
                    ...
                    console.log('Route API response:', JSON.stringify(routes));
                    this._processRoutes(routes, requestedCallsigns);
                } catch (e) {
    ...
    _processRoutes(routes, requestedCallsigns) {
        ...
        for (const routeData of routes) {
            if (routeData && routeData.callsign && routeData._airport_codes_iata) {
                ...
                // Skip routes the API isn't confident about (handles both boolean and numeric)
                if (routeData.plausible === 0 || routeData.plausible === false) {
                    console.log(`Route API low confidence, skipping: ${callsign} -> ${route}`);
                    continue;
                }

                // Only cache routes for callsigns we actually requested
                if (requestedCallsigns && !requestedCallsigns.has(callsign)) {
                    console.warn(`Route API returned unrequested callsign: ${callsign} -> ${route}`);
                    continue;
                }

                // Store route with timestamp for TTL tracking
                this.routeCache.set(callsign, { route, timestamp: now });
